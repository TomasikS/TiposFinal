  ...
  public void logout(String login, String token) {
    ...
    public boolean checkIfEmailOrLoginExist(String login, String email) {
        try {
            ...
            String query = "SELECT count(*) as num FROM user WHERE login like ? OR email like ?";
            PreparedStatement ps = (PreparedStatement) conn.prepareStatement(query);
            ps.setString(1,login);
    ...
    public void insertNewUserIntoDb(Registration registration) {
        try {
            Class.forName(driver).newInstance();
            conn = DriverManager.getConnection(url, this.username, this.password);
            String query = "INSERT INTO user(firstname, lastname, email, login, password) "+
                    " VALUES (?,?,?,?,?)";
            PreparedStatement ps= (PreparedStatement) conn.prepareStatement(query);
    ...
    public void insertBets(Ticket ticket) {
        try {
            ...
            String query = "INSERT INTO bet (idu) SELECT id FROM user where login like ?";
            PreparedStatement ps = (PreparedStatement) conn.prepareStatement(query);

            ps.setString(1,ticket.login);
            System.out.println(ps);
            ps.executeUpdate();
            query="SELECT max(id) as max from bet where idu = (SELECT id FROM user where login like ?)";
            ps = (PreparedStatement) conn.prepareStatement(query);
            ps.setString(1,ticket .login);
            System.out.println(ps);
            ResultSet rs = (ResultSet) ps.executeQuery();
    ...
    public List getActualTickets(String username) {
        try {
            ...
            String query = "SELECT * from bet "
                    +" INNER JOIN users ON user.id=bet.idu "
